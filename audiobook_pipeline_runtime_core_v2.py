...
DEFAULT_RUNTIME_CONFIG = {'SUPABASE_URL': '',
 'SUPABASE_KEY': '',
 ...
 'MODELSCOPE_TOKEN_SOURCE': 'supabase',
 ...
 'HF_DATASET_ZIP_URLS_SOURCE': 'supabase',
 'HF_DATASET_ZIP_URLS': '',
 'BUCKET_IDS_SOURCE': 'supabase',
 'BUCKET_IDS': '',
 'HF_TOKEN': '',
...
def load_cloud_music_runtime_setting(setting_key):
    ...
    try:
        ...
        if not str(globals().get("SUPABASE_URL", "") or "").strip() or not str(globals().get("SUPABASE_KEY", "") or "").strip():
            return ""
        ...
        # 兼容旧版按频道保存的数据：若共享作用域还没有值，回退到最新一条并复用
        legacy_rows = (
            client.table(table_name)
            .select("setting_value")
            .eq("setting_key", setting_key)
        ...
        if legacy_rows:
            print(f"⚠️ 共享云端配置 {setting_key} 暂未设置，当前临时回退到旧数据中的最新一条。")
            return str(legacy_rows[0].get("setting_value") or "")

        return ""
    except Exception as e:
        print(f"⚠️ 读取云端运行配置 {setting_key} 失败，先回退本地值: {e}")
        return ""


def resolve_music_runtime_setting(setting_key, local_value, source="supabase"):
    mode = str(source or "supabase").strip().lower()
    local_text = str(local_value or "")

    if mode not in {"supabase", "local"}:
        print(f"⚠️ {setting_key} 的来源配置无效，当前回退本地值。")
        return local_text
    ...
    if str(cloud_value).strip():
        print(f"☁️ 已从 Supabase 读取 {setting_key}")
        return str(cloud_value)

    if str(local_text).strip():
        print(f"⚠️ Supabase 中未找到 {setting_key}，当前运行临时回退到本地值。")
    return local_text
...
def apply_music_download_runtime_overrides():
    globals()["HF_DATASET_ZIP_URLS"] = resolve_music_runtime_setting(
        ...
        globals().get("HF_DATASET_ZIP_URLS_SOURCE", "supabase"),
    )
    globals()["BUCKET_IDS"] = resolve_music_runtime_setting(
        "BUCKET_IDS",
        globals().get("BUCKET_IDS", ""),
        globals().get("BUCKET_IDS_SOURCE", "supabase"),
    )
...
def require_supabase_client():
    ...
    if client is None:
        raise RuntimeError("Supabase client 未初始化，请先运行主流程单元。")
    return client
...
def load_modelscope_token_from_supabase(channel_name=None):
    ...
    try:
        ...
        rows = response.data or []
        if rows:
            return str(rows[0].get("token_text") or "").strip()
        ...
        if channel:
            ...
            if legacy_rows:
                return str(legacy_rows[0].get("token_text") or "").strip()
        ...
        if fallback_rows:
            return str(fallback_rows[0].get("token_text") or "").strip()
        return ""
    except Exception as e:
        raise RuntimeError(f"从 Supabase 读取 ModelScope Token 失败，请检查表 {table_name}: {e}")
...
def save_modelscope_token_to_supabase(channel_name, token_text):
    ...
    if not token_value:
        raise RuntimeError("MODELSCOPE_TOKEN 为空，无法写入 Supabase")

    payload = {
        "channel_name": shared_scope,
    ...
    try:
        ...
        raise RuntimeError(f"写入 Supabase ModelScope Token 失败，请检查表 {table_name}: {e}")

    return f"supabase:{table_name}:{shared_scope}"
...
def delete_modelscope_token_from_supabase(channel_name):
    ...
    shared_scope = get_shared_cloud_runtime_scope_key()
    try:
        client.table(table_name).delete().eq("channel_name", shared_scope).execute()
        return True
    except Exception as e:
        raise RuntimeError(f"删除 Supabase ModelScope Token 失败，请检查表 {table_name}: {e}")
...
def load_cloud_runtime_setting_from_supabase(channel_name, setting_key):
    ...
    try:
        ...
        if fallback_rows:
            return str(fallback_rows[0].get("setting_value") or "")
        return ""
    except Exception as e:
        raise RuntimeError(f"从 Supabase 读取云端运行配置 {key} 失败，请检查表 {table_name}: {e}")
...
def save_cloud_runtime_setting_to_supabase(channel_name, setting_key, setting_value):
    ...
    if not key:
        raise RuntimeError("setting_key 为空，无法将云端运行配置写入 Supabase")
    ...
    try:
        ...
        raise RuntimeError(f"写入 Supabase 云端运行配置 {key} 失败，请检查表 {table_name}: {e}")

    return f"supabase:{table_name}:{shared_scope}:{key}"
...
def delete_cloud_runtime_setting_from_supabase(channel_name, setting_key):
    ...
    table_name = get_cloud_runtime_settings_table_name()
    try:
        client.table(table_name).delete().eq("channel_name", shared_scope).eq("setting_key", key).execute()
        return True
    except Exception as e:
        raise RuntimeError(f"删除 Supabase 云端运行配置 {key} 失败，请检查表 {table_name}: {e}")


def resolve_cloud_text_setting(setting_key, local_value="", source="supabase", channel_name=None):
    mode = str(source or "supabase").strip().lower()
    local_text = str(local_value or "")

    if mode not in {"supabase", "local"}:
        raise RuntimeError(f"{setting_key} 的来源配置只能是 'supabase' 或 'local'")
    ...
    try:
        ...
        log.warning("读取云端运行配置 %s 失败，当前回退到本地值: %s", setting_key, e)
        return local_text

    if str(cloud_value).strip():
        log.info("已从 Supabase 读取全局共享云端配置 %s", setting_key)
        return str(cloud_value)

    if str(local_text).strip():
        log.warning(
            "Supabase 中未找到全局共享云端配置 %s，当前运行临时回退到本地值；如需持久保存，请手动开启云端运行配置同步单元",
            setting_key,
        )
        return local_text
...
def resolve_modelscope_token(channel_name=None):
    source = str(globals().get("MODELSCOPE_TOKEN_SOURCE", "supabase") or "supabase").strip().lower()
    local_token = str(globals().get("MODELSCOPE_TOKEN", "") or "").strip()
    channel = str(channel_name or globals().get("YOUTUBE_CHANNEL_NAME", "") or "").strip()

    if source not in {"supabase", "local"}:
        raise RuntimeError("MODELSCOPE_TOKEN_SOURCE 只能是 'supabase' 或 'local'")
    ...
    if source == "local":
        ...
        log.info("已按本地模式读取 MODELSCOPE_TOKEN，并同步回写到 Supabase 全局共享云端配置")
        return local_token
    ...
    if cloud_token:
        log.info("已从 Supabase 读取全局共享 ModelScope Token")
        return cloud_token
    ...
    if legacy_token:
        ...
        log.info("已从 Supabase 旧表读取 ModelScope Token，并补写到全局共享云端配置表")
        return legacy_token

    if local_token:
        save_cloud_runtime_setting_to_supabase(channel, "MODELSCOPE_TOKEN", local_token)
        save_modelscope_token_to_supabase(channel, local_token)
        log.warning("Supabase 中未找到全局共享 ModelScope Token，已自动用本地 MODELSCOPE_TOKEN 回填云端")
        return local_token

    raise RuntimeError(
        "Supabase 中未找到全局共享 ModelScope Token，且本地 MODELSCOPE_TOKEN 也为空，无法继续 AI 生成"
    )


def build_split_state_ref(book_id, project_flag=None):
    flag = str(PROJECT_FLAG if project_flag is None else project_flag).strip()
    return f"supabase:{get_book_state_table_name()}:{flag}:{book_id}"
...
def load_split_processing_state(book_record):
    ...
    try:
        response = (
            client.table(table_name)
            .select(
                "book_id, project_flag, book_name, category, pending_resume, state_status, "
                "current_part_index, completed_part_count, part_count, updated_at, created_at, state_json"
            )
            .eq("book_id", book_id)
            .eq("project_flag", project_flag)
            .order("updated_at", desc=True)
            .limit(1)
            .execute()
        )
        ...
        if not rows:
            return None
        return normalize_split_state_from_row(rows[0])
    except Exception as e:
        raise RuntimeError(f"从 Supabase 读取断点状态失败，请检查表 {table_name}: {e}")
...
def save_split_processing_state(book_record, state):
    ...
    payload = {
        ...
        "current_part_index": state.get("current_part_index"),
        "completed_part_count": int(state.get("completed_part_count") or 0),
        "part_count": int(state.get("part_count") or 1),
    ...
    try:
        ...
        raise RuntimeError(f"写入 Supabase 断点状态失败，请检查表 {table_name}: {e}")
...
def delete_split_processing_state(book_record, only_if_completed=False):
    ...
    try:
        ...
        if only_if_completed:
            query = query.eq("state_status", "completed")
        query.execute()
        return True
    except Exception as e:
        raise RuntimeError(f"删除 Supabase 分片状态失败，请检查表 {table_name}: {e}")
    ...
    while True:
        try:
            query = (
                ...
                .eq("state_status", "completed")
                .order("updated_at", desc=False)
                .limit(100)
            )
            if flag:
                query = query.eq("project_flag", flag)
            if category_name:
                query = query.eq("category", category_name)
            rows = query.execute().data or []
        except Exception as e:
            raise RuntimeError(f"清理 Supabase 已完成分片状态失败，请检查表 {table_name}: {e}")
        ...
        for row in rows:
            ...
            try:
                delete_query = (
                    client.table(table_name)
                    .delete()
                    .eq("book_id", current_book_id)
                    .eq("project_flag", current_flag)
                    .eq("state_status", "completed")
                )
...
def apply_cloud_runtime_overrides():
    ...
    try:
        ...
        resolved_dataset_urls = resolve_cloud_text_setting(
            ...
            source=globals().get("HF_DATASET_ZIP_URLS_SOURCE", "supabase"),
        )
    ...
    try:
        ...
        resolved_bucket_ids = resolve_cloud_text_setting(
            ...
            source=globals().get("BUCKET_IDS_SOURCE", "supabase"),
        )
    ...
    while True:
        try:
            query = (
                ...
                .eq("project_flag", project_flag)
                .eq("pending_resume", True)
                .order("updated_at", desc=True)
            )
            if TARGET_CATEGORY.strip():
                query = query.eq("category", TARGET_CATEGORY.strip())

            rows = query.range(offset, offset + page_size - 1).execute().data or []
        except Exception as e:
            raise RuntimeError(f"查询 Supabase 未完成断点状态失败，请检查表 {table_name}: {e}")
...
def validate_runtime_config():
    ...
    modelscope_token_source = str(MODELSCOPE_TOKEN_SOURCE or "supabase").strip().lower()
    local_modelscope_token = str(MODELSCOPE_TOKEN or "").strip()
    hf_dataset_zip_urls_source = str(HF_DATASET_ZIP_URLS_SOURCE or "supabase").strip().lower()
    bucket_ids_source = str(BUCKET_IDS_SOURCE or "supabase").strip().lower()

    if not str(SUPABASE_URL).strip():
        errors.append("SUPABASE_URL 为空")
    if not str(SUPABASE_KEY).strip():
        errors.append("SUPABASE_KEY 为空")
    ...
    if DOWNLOAD_FROM_BUCKETS:
        if hf_dataset_zip_urls_source not in {"supabase", "local"}:
            errors.append("HF_DATASET_ZIP_URLS_SOURCE 只能是 supabase 或 local")
        if bucket_ids_source not in {"supabase", "local"}:
            errors.append("BUCKET_IDS_SOURCE 只能是 supabase 或 local")
        if music_download_method not in {"datasets_zip_urls", "buckets"}:
            ...
            elif hf_dataset_zip_urls_source == "supabase" and not str(HF_DATASET_ZIP_URLS).strip():
                warnings.append(
                    f"HF_DATASET_ZIP_URLS 默认读云端，且本地值为空；请确保 Supabase 的 {CLOUD_RUNTIME_SETTINGS_TABLE} 表里已写入全局共享 HF_DATASET_ZIP_URLS"
                )
        else:
            ...
            elif bucket_ids_source == "supabase" and not bucket_ids:
                warnings.append(
                    f"BUCKET_IDS 默认读云端，且本地值为空；请确保 Supabase 的 {CLOUD_RUNTIME_SETTINGS_TABLE} 表里已写入全局共享 BUCKET_IDS"
                )
    ...
    if ai_features_enabled:
        if modelscope_token_source not in {"supabase", "local"}:
            errors.append("MODELSCOPE_TOKEN_SOURCE 只能是 supabase 或 local")
        ...
        if modelscope_token_source == "supabase" and not local_modelscope_token:
            warnings.append(
                f"MODELSCOPE_TOKEN_SOURCE=supabase 且本地 MODELSCOPE_TOKEN 为空；请确保 Supabase 的 {MODELSCOPE_TOKEN_TABLE} 或 {CLOUD_RUNTIME_SETTINGS_TABLE} 表中已写入全局共享 token"
            )
...
def _build_youtube_cover_draw_prompt(book_name, book_desc, current_token, attempt):
    ...
    system_prompt = """角色设定：
你现在是一位顶级的YouTube视频封面设计师和AI绘画提示词专家。你的任务是根据我提供的【书名】和【内容简介】，为我设计一个极具点击率（CTR）的YouTube视频封面，该封面必须包含清晰、巨大的中文书名文字。请直接输出可用于Midjourney（v6+）、DALL-E 3或类似高级模型的纯英文绘图提示词。

设计原则：
1. 醒目且融合的中文文本： 书名中文文字必须巨大、清晰，并作为整个画面设计的核心视觉元素（Verbatim）。指定字体风格，并确保文字与背景有极高的对比度。
2. 高度相关的核心图像： 主体必须直接体现书的内容，传达强烈情感（神秘、兴奋、恐惧等），具备对角线、中央特写等YouTube Thumbnail 高点击构图；高饱和度且具光影渲染。
3. 可选“钩子”文本： 允许在书名旁添加极短的中文副标题（不超过5个字）增强吸引力。

最后约束（非常重要）：
1. 你的回答必须只能是一段纯粹的英文 Prompt 提示词字符串（必须包含 `--ar 16:9`）。
2. 绝对不能有任何中文分析、构思或者“好的、收到”等前导回复！
3. 不要使用列表、标号或其他结构，直接输出全案描绘。"""
    ...
    draw_prompt = response.choices[0].message.content.strip()
...
def _request_modelscope_cover_image_url(image_model, current_token, draw_prompt, img_size):
    ...
    while polls < max_polls:
        ...
        if status == "SUCCEED":
            ...
            log.info("📥 远端结壳完毕！获取到高速下载热池锚点: %s", img_url)
            return img_url
        if status == "FAILED":
            raise ValueError(f"{image_model} 远端画图机返回 FAILED！")

        time.sleep(poll_interval)

    raise ValueError(f"由于排队压力远端超时 {max_polls * poll_interval} 秒仍未能接头画完图！")


def _try_generate_cover_with_image_model(output_path, draw_prompt, img_size, image_model, token_candidates):
    remaining_tokens = list(token_candidates)
    non_quota_errors = []

    for token_index, current_token in enumerate(list(remaining_tokens), start=1):
        try:
            ...
            if download_file(img_url, output_path):
                ...
                return True, remaining_tokens, None

            raise ValueError("URL 下载至本地图盘时文件被截断了")
        except Exception as e:
            if is_modelscope_daily_quota_exceeded_error(e):
                remaining_tokens = [item for item in remaining_tokens if item != current_token]
                log.warning(
                    "⚠️ %s 第 %d 个 token 失败：当前 token 今日额度已用尽，切换下一个 token。剩余可用 token=%d | 原始错误：%s",
                    image_model,
                    token_index,
                ...
                if not remaining_tokens:
                    raise RuntimeError("所有 ModelScope Token 都已触发今日额度上限，停止 AI 封面生成。") from e
                time.sleep(2)
                continue

            non_quota_errors.append(str(e))
            log.warning(
    ...
    return False, remaining_tokens, non_quota_errors
...
def auto_create_youtube_cover(book_name, book_desc, output_path, token, resolution="1080p"):
    ...
    attempt = 0
    active_token_pool = list(token_pool)
    ...
    while True:
        if not active_token_pool:
            raise RuntimeError("所有 ModelScope Token 都已触发今日额度上限，停止 AI 封面生成。")

        attempt += 1
        current_cycle_errors = []
        draw_prompt, prompt_errors = _run_qwen_task_with_token_rotation(
            task_label="封面提示词生成",
            token_pool=active_token_pool,
            attempt=attempt,
        ...
        if not draw_prompt:
            log.warning(
                "⚠️ 封面生成模块第 %d 次失败：当前可用 token 全部未能生成提示词。错误摘要：%s；系统将持续重试，直到成功为止。",
                attempt,
        ...
        for model_index, (image_model, model_label) in enumerate(image_model_sequence, start=1):
            success, active_token_pool, model_errors = _try_generate_cover_with_image_model(
                output_path=output_path,
                draw_prompt=draw_prompt,
                img_size=img_size,
                image_model=image_model,
                token_candidates=active_token_pool,
            )
            if success:
                return True

            if model_errors:
                current_cycle_errors.extend([f"{image_model}: {msg}" for msg in model_errors])
        ...
        log.warning(
            "⚠️ 封面生成模块第 %d 次失败：Qwen 生图与 Tongyi 回退生图都已在当前可用 token 上完整尝试仍未成功。错误摘要：%s；系统将持续重试，直到成功为止。",
            attempt,
...
def authenticate_youtube_from_supabase(channel_name):
    """从 Supabase 获取指定频道的 YouTube Token，并在需要时自动刷新。"""
    log.info("🔐 正在连线 Supabase 拉取 YouTube '%s' 频道无人值守通行证...", channel_name)
    try:
        response = supabase.table("youtube_credentials").select("token_json").eq("channel_name", channel_name).execute()
        if not response.data:
            log.error("❌ 无法在 Supabase 找到频道 %s 的授权凭证。请去平台预先配置添加！", channel_name)
            return None

        token_info = response.data[0].get("token_json")
        if not token_info:
        ...
        if credentials.expired:
            if credentials.refresh_token:
                ...
                try:
                    supabase.table("youtube_credentials").update(
                        {"token_json": refreshed_token}
                    ).eq("channel_name", channel_name).execute()
                    log.info("✅ 新令牌已自动回写 Supabase。")
                except Exception as refresh_save_error:
                    log.warning("⚠️ 令牌刷新成功，但回写 Supabase 失败: %s", refresh_save_error)
            else:
                log.error("❌ YouTube 凭证已过期，且缺少 refresh_token，无法自动刷新。")
                return None

        youtube = build("youtube", "v3", credentials=credentials, cache_discovery=False)
        log.info("✅ YouTube '%s' 频道连线并授权成功！正在为它启动网闸...", channel_name)
        return youtube
    except Exception as e:
        log.error("❌ 初始化 YouTube 客户端失败，检查 Supabase 连接和表数据: %s", e)
        return None
...
def build_youtube_status(privacy_status="unlisted", schedule_after_hours=0):
    ...
    if normalized == "schedule":
        ...
        log.info("📅 YouTube 预约公开已启用：%d 小时后自动公开 (%s)", hours, publish_at)
        return {
...
def _build_video_upload_request_body(title, description, tags, privacy_status="unlisted", category_id="", schedule_after_hours=0):
    ...
    return {
        "snippet": snippet,
        "status": build_youtube_status(privacy_status, schedule_after_hours),
    }
...
def restore_split_shared_assets_from_state(result, state, book_dir, safe_name, book_name):
    ...
    if seo_title or seo_description or seo_tags:
        ...
        try:
            ...
            log.warning("[%s] 从 Supabase 恢复 SEO 文案失败: %s", book_name, e)
    ...
    if cover_base64:
        ...
        try:
            ...
            log.warning("[%s] 从 Supabase 恢复共享封面失败: %s", book_name, e)

    if restored_items:
        log.info("[%s] 已从 Supabase 恢复长音频共享%s。", book_name, "与".join(restored_items))
...
def persist_split_shared_assets_to_state(book_record, state, result, book_dir, safe_name, book_name):
    ...
    if result.cover_image_path and os.path.exists(result.cover_image_path) and os.path.getsize(result.cover_image_path) > 0:
        try:
            ...
            log.warning("[%s] 写入 Supabase 共享封面前读取本地文件失败: %s", book_name, e)
...
def cleanup_completed_split_state_for_book(book_record, result, book_name):
    try:
        if delete_split_processing_state(book_record, only_if_completed=True):
            result.state_path = ""
            log.info("[%s] 长音频分片状态已完成并从 Supabase 清理，避免继续占用空间。", book_name)
    except Exception as e:
...
def process_book(book_record: dict, run_started_at=None) -> BookResult:
    ...
    3. 分片模式只处理当前分片所需章节，并把状态写入 Supabase 的 BOOK_STATE_TABLE。
    """
...
def run_pipeline(runtime_config: dict | None = None):
    ...
    if deleted_completed_states > 0:
        log.info("已清理 %d 条已完成的长音频分片状态，避免占用 Supabase 空间", deleted_completed_states)
    ...
    while True:
        ...
        query = supabase.table("books").select(
            "book_id, book_name, category, book_data, status, tags"
        ).order("book_id")
            ...
            if result.success:
                ...
                try:
                    supabase.table("books").update(
                        {"status": new_status}
                    ).eq("book_id", book["book_id"]).execute()
                ...
                # 如果是无用数据(chapters_data 为空)，则在 tags 中追加 "bad"
